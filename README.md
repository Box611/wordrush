...
| 资源 | 原计划用途 | 放弃原因 |
|---|---|---|
| **Tatoeba API** | 取例句 | ⚠️ **响应头里没有 `Access-Control-Allow-Origin`**，浏览器会直接拦截跨域请求 —— 纯前端**根本调不通**。另外单次请求耗时 15 秒。想用只能自建后端代理，与「零后端」定位冲突。 |
| **Hugging Face 数据集**（cc-news-sample 1300 万篇、BBC News 2224 篇） | 分级阅读材料 | 体积以 GB 计，且需要 Python/datasets 环境下载。纯静态站点无法承载。**替代方案**：挑几篇粘贴进「自定义文本」，或写进 `data/sources.js`。 |
| **ECDICT（77 万词条）** | 词典数据 | 全量 CSV 超过 200 MB，内嵌进一个静态网站完全不现实。若只要音标，本项目的词库已经带了。 |
| **isdict-api** | 权威释义 | 需要申请 API Key，且主要用于服务端调用；本项目定位是「打开即用、无需配置」。 |
| **KyleBing/english-vocabulary** | 四六级词库 | 仓库本身可用（README 能拉到），但**文件目录接口返回 403**，且 JSONL 路径带中文目录名，实测多次请求失败，无法稳定获取数据。 |
| **lilinji/English** | 分类词书 | 只有 `.xlsx` 格式，需要 Excel 才能拆解，不适合作为自动化数据源。 |
